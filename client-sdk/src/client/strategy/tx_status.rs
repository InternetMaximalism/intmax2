...
pub async fn get_tx_status<V: ValidityProverClientInterface>(
    ...
    let sender_leaves = validity_prover
        ...
        .await?
        .ok_or(StrategyError::UnexpectedError(
            "sender leaves not found".to_string(),
        ))?;
    let sender_leaf = sender_leaves.into_iter().find(|leaf| leaf.sender == sender);
    ...
    let validity_pis = validity_prover
        ...
        .await?
        .ok_or(StrategyError::UnexpectedError(
            "validity pis not found".to_string(),
        ))?;
