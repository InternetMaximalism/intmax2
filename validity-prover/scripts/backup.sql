...
VALUES (TRUE, (SELECT val FROM _temp_final_new_cutoff))
ON CONFLICT (singleton_key) DO UPDATE
SET block_number = EXCLUDED.block_number;

RAISE NOTICE 'Cutoff value in cutoff table ensured to be %', (SELECT val FROM _temp_final_new_cutoff);

\echo '--- Copying data up to new cutoff ---'
\set new_cutoff_value `p_one "SELECT val FROM _temp_final_new_cutoff"`
...
FROM hash_nodes
WHERE tag = :'ORIGINAL_TAG_1' AND timestamp <= :'new_cutoff_value'
...
FROM hash_nodes
WHERE tag = :'ORIGINAL_TAG_2' AND timestamp <= :'new_cutoff_value'
ON CONFLICT (tag, timestamp, bit_path) DO NOTHING;
...
FROM hash_nodes
WHERE tag = :'ORIGINAL_TAG_3' AND timestamp <= :'new_cutoff_value'
ON CONFLICT (tag, timestamp, bit_path) DO NOTHING;
