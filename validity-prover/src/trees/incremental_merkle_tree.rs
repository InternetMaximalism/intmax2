...
use crate::trees::merkle_tree::{HashOut, IncrementalMerkleTreeClient, MTResult};

#[derive(Debug, Clone)]
pub struct HistoricalIncrementalMerkleTree<
    V: Leafable + Serialize + DeserializeOwned,
    DB: IncrementalMerkleTreeClient<V>,
> {
    merkle_tree: DB,
    _phantom: std::marker::PhantomData<V>,
}

impl<V: Leafable + Serialize + DeserializeOwned, DB: IncrementalMerkleTreeClient<V>>
    HistoricalIncrementalMerkleTree<V, DB>
{
    ...
    pub async fn len(&self, timestamp: u64) -> MTResult<usize> {
        let len = self.merkle_tree.len(timestamp).await?;
        Ok(len)
    }
