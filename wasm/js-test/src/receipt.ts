...
async function main() {
    ...
    const receipt = await generate_transfer_receipt(config, account.view_pair, tx_digest, transfer_index);
    console.log(`size of receipt: ${receipt.length}`);

    // verify the receipt
    const recovered_transfer_data = await validate_transfer_receipt(config, account.view_pair, receipt)
